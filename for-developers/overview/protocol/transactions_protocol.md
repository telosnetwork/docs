...
1. Explicit actions, which are present in a signed transaction (see [2. Transaction Instance](transactions\_protocol.md#2-transaction-instance)).
...
| `actions`                | array of `action`         | list of [action instances](transactions\_protocol.md#3-4-3-action-instance) |
...
![](<../../../.gitbook/assets/graphviz (4).svg>)

#### 3.4.4. Authority Check

After the minimum permission levels are checked, the authority table for the receiver account’s permission that matches each actor’s permission within the action instance is checked (see [Accounts and Permissions: 3. Permissions](accounts\_and\_permissions.md#3-permissions) for more details).
